...
//set work, isneedhelp, isneedcalling = 0

function resetAllStatus(bid){
    if(bid != null){
        pool.getConnection(function(err, connection){
            if(err) console.error(err);
            else{
                connection.query('update blindgo.blinduser set work=0,isneedhelp=0,isneedcalling=0 where BID = ?',
                    [bid.toString()],function(err){
                        if(err) console.error(err);
                    });
